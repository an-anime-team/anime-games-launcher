...
Most of the APIs use special `Bytes` usertype which stored immutable slice of
binary data on the runtime side and allows lua side to read them. Lua types
such as strings or tables with sequences of numbers can be converted into
`Bytes` too.
...
Modules permissions can be specified using modules scopes lists or directly in
the game package lock file generated by the launcher.
...
> actual functions implementations in `agl-runtime/src/api` folder.
